...
Loop
	...
	memory_buffer
		...
		addwfc value_3, W ; W = x[n] + x[n-6] + x[n-3]
	...
	adder_divider
		addwfc value_b ; add current contents of W = x[n] + x[n-6] + x[n-3] into value_b = x[n-9]
		rrcf value_b ; value_b = (x[n] + x[n-9] + x[n-6] + x[n-3])/2
		rrcf value_b ; value_b = (x[n] + x[n-9] + x[n-6] + x[n-3])/4
