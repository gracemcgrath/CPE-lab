...
Loop
	...
	memory_buffer
		movff value_10, value_c ; save x[n-10] into c[n]
		movff value_9, value_10 ; save x[n-9] into x[n-10]
		movff value_8, value_9 ; save x[n-8] into x[n-9]
		addwfc value_7, W ; W = x[n] + x[n-7]
		movff value_7, value_8 ; save x[n-7] into x[n-8]
		movff value_6, value_7 ; save x[n-6] into x[n-7]
		addwfc value_5, W ; W = x[n] + x[n-7] + x[n-5]
		movff value_5, value_6 ; save x[n-5] into x[n-6]
	...
	adder_divider
		addwfc value_c ; add current contents of  W = x[n] + x[n-7] + x[n-5] into value_c = x[n-1-]
		rrcf value_c ; value_c = (x[n] + x[n-10] + x[n-7] + x[n-5])/2
		rrcf value_c ; value_c = (x[n] + x[n-10] + x[n-7] + x[n-5])/4
